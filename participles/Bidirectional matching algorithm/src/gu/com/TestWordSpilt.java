...
public class TestWordSpilt {
     ...
     * @throws IOException
     */
    public TestWordSpilt() throws IOException {
        BufferedReader br = new BufferedReader(new FileReader("E:\\Mark_of_growth\\participles\\Bidirectional matching algorithm\\src\\dict.txt"));
        String line = null;
    ...
    public List<String> spilt(String spiltStr, boolean leftToRight) {
        ...
        while (length < spiltStr.length()) {
            ...
            if (spiltStr.length() - length < MAX_LENGTH) wordLength = spiltStr.length() - length;
                // 否则取默认值
            else wordLength = MAX_LENGTH;
            ...
            while (!map.containsKey(word)) {
                ...
                if (word.length() == 1) {
                    ...
                    if (word.matches("[a-zA-z0-9]")) {
                        ...
                        if (leftToRight) {
                            //indexOf返回指定子字符串在此字符串中第一次出现处的索引，从指定的索引开始
                            //position
    ...
    System.out.println("\n双向分词结果：");*/
        ...
        while (left.get(0).length() + right.get(right.size() - 1).length() < inputStr.length()) {
            ...
            inputStr = inputStr.substring(0, inputStr.length() - right.get(right.size() - 1).length());
            // 清理之前正逆向分词结果，防止造成干扰
            left.clear();
            right.clear();
            // 对没被吃掉的字符串重新开始分词
        ...
        if (left.get(0).length() + right.get(right.size() - 1).length() > inputStr.length()) resultMiddle = left;
        // 如果中间交汇，刚好吃完，没有重叠：
        // 正向第一个分词 + 反向最后一个分词的长度 = 输入字符串总长度，那么正反向一拼即可
        if (left.get(0).length() + right.get(right.size() - 1).length() == inputStr.length()) {
    ...
    public static void main(String[] args) {
        // example:欢迎来到脚本之家，这里提供大量的脚本素材提供读者下载，参考
        Scanner input = new Scanner(System.in);
        String str = input.nextLine();
        TestWordSpilt wordSpilt = null;
        try {
            wordSpilt = new TestWordSpilt();
        } catch (IOException e) {
            e.printStackTrace();
        }
